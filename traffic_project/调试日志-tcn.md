1.
   ...
   2.
       11.7:上面是对模型框架的优化，现在进行超参调优和特征工程阶段：
           ...
           （2）调整模型“宽度” (Channel Width)从我的原始图表（紫色曲线太平滑）来看，模型可能仍然欠拟合 (Underfit) 高频峰值。这意味着模型的容量 (Capacity) 可能不足，无法记住那些复杂、尖锐的模式。
                改进：TCN_CHANNELS = [128, 128, ...] -> TCN_CHANNELS = [256, 256, ...]
                效果：11.68% -> 12.17% 验证MAPE变高，效果变差，应该是模型太复杂了，有点稍微的过拟合了。
                具体数据：
                Epoch 23/50 - Train Loss: 0.0017 (Log-Space), Val Loss: 0.0061 (Log-Space), Val MAPE: 11.68% (Original-Space)
                ✨ 新的最佳 TCN 模型 (MAPE: 11.68%) 已保存!
                ->
                Epoch 25/50 - Train Loss: 0.0016 (Log-Space), Val Loss: 0.0067 (Log-Space), Val MAPE: 12.17% (Original-Space)
                ✨ 新的最佳 TCN 模型 (MAPE: 12.17%) 已保存!放弃！
           （3）调整尝试不同的学习率调度器 (Scheduler) ReduceLROnPlateau 是一个“被动”的调度器（它需要等待验证损失停止下降）。它依赖patience 参数，而这个参数很难调。
                具体修改：
                更换为CosineAnnealingLR学习率调度器，它会在 EPOCHS 这么长的时间内，将学习率从 1e-3 平滑地降低到 1e-6（像一个余弦曲线的四分之一）。这在 TCN 和 Transformer 中经常有奇效。
                效果：11.68% -> 12.67%
                具体数据：
                Epoch 23/50 - Train Loss: 0.0017 (Log-Space), Val Loss: 0.0061 (Log-Space), Val MAPE: 11.68% (Original-Space)
                ✨ 新的最佳 TCN 模型 (MAPE: 11.68%) 已保存!
                ->
                Epoch 20/50 - Train Loss: 0.0018 (Log-Space), Val Loss: 0.0068 (Log-Space), Val MAPE: 12.67% (Original-Space)
                ✨ 新的最佳 TCN 模型 (MAPE: 12.67%) 已保存!
                效果变差放弃。
           （4）综合改进：DROPOUT = 0.5 + TCN_CHANNELS = [128, 128, ...] -> TCN_CHANNELS = [256, 256, ...]
                效果：11.68% -> 11.60% 效果变好了 模型5
                具体数据：
                Epoch 23/50 - Train Loss: 0.0017 (Log-Space), Val Loss: 0.0061 (Log-Space), Val MAPE: 11.68% (Original-Space)
                ✨ 新的最佳 TCN 模型 (MAPE: 11.68%) 已保存!
                ->
                Epoch 27/50 - Train Loss: 0.0017 (Log-Space), Val Loss: 0.0062 (Log-Space), Val MAPE: 11.60% (Original-Space)
                ✨ 新的最佳 TCN 模型 (MAPE: 11.60%) 已保存!
